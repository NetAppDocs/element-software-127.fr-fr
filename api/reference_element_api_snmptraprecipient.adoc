...
[role="lead"]
L'objet snmpTrapRecent contient des informations sur un hôte configuré pour recevoir les traps SNMP générés par le cluster de stockage. Vous pouvez utiliser la `GetSnmpTrapInfo` méthode API pour obtenir une liste d'hôtes configurés pour recevoir des traps SNMP.
