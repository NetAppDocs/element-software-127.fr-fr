...
[role="lead"]
L'objet session contient des informations sur chaque session Fibre Channel visible pour le cluster et sur les ports cibles sur lesquels il est visible. Vous pouvez récupérer ces informations avec la `ListFibreChannelSessions` méthode API.
