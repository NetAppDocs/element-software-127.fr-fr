...
[role="lead"]
Vous pouvez utiliser la `AddVirtualNetwork` méthode pour ajouter un nouveau réseau virtuel à une configuration de cluster.
...
NOTE: La méthode AddVirtualNetwork est utilisée uniquement pour créer un nouveau réseau virtuel. Si vous souhaitez apporter des modifications à un réseau virtuel existant, utilisez la xref:reference_element_api_modifyvirtualnetwork.adoc[ModilyVirtualNetwork] méthode.
