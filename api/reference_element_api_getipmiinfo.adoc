...
[role="lead"]
Vous pouvez utiliser cette `GetIpmiInfo` méthode pour afficher un rapport détaillé sur les capteurs (objets) des ventilateurs de nœud, les températures d'admission et d'échappement, ainsi que les alimentations surveillées par le système.
