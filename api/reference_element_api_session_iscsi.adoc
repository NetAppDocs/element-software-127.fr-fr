...
[role="lead"]
L'objet session (iSCSI) contient des informations détaillées sur la session iSCSI de chaque volume. Vous pouvez récupérer les informations de session iSCSI avec la `ListISCSISessions` méthode API.
