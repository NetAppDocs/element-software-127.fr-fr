...
[role="lead"]
Vous pouvez utiliser la `AddClusterAdmin` méthode pour ajouter un nouveau compte admin du cluster. Un administrateur de cluster peut gérer le cluster via l'API et les outils de gestion. Les administrateurs de clusters sont entièrement séparés et n'ont pas de rapport avec les comptes de tenant standard.
