...
[role="lead"]
Vous pouvez utiliser la `TestDrives` méthode pour exécuter une validation matérielle sur tous les disques du nœud. Cette méthode détecte les défaillances matérielles sur les lecteurs et signale les résultats des tests de validation.



== Paramètres

Vous pouvez utiliser la méthode uniquement `TestDrives` sur les nœuds qui ne sont pas « actifs » dans un cluster.
