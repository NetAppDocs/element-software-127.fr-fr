...
[role="lead"]
Vous pouvez utiliser la `GetVolumeAccessGroupEfficiency` méthode pour obtenir des informations sur l'efficacité d'un groupe d'accès de volume. Seul le groupe d'accès aux volumes que vous fournissez comme paramètre dans cette méthode API est utilisé pour calculer la capacité.
