...
[role="lead"]
Vous pouvez utiliser `TestRemoteConnectivity` la méthode pour envoyer une requête ping à chaque nœud de la grappe distante et vérifier la connexion à la base de données de l’ensemble distant. Les clusters doivent être couplés pour renvoyer des résultats utiles avec cette méthode. Si la connexion à la base de données distante échoue, la réponse du système répertorie les exceptions.
