...
[role="lead"]
Vous pouvez utiliser cette `ResetDrives` méthode pour initialiser de façon proactive les lecteurs et supprimer toutes les données résidant actuellement sur un lecteur. Le disque peut ensuite être réutilisé dans un nœud existant ou utilisé dans un nœud mis à niveau.
