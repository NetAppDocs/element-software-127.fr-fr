...
[role="lead"]
Vous pouvez utiliser la `ListVirtualVolumeHosts` méthode pour obtenir une liste de tous les hôtes de volume virtuel connus du cluster. Un hôte de volume virtuel est un hôte VMware ESX qui a démarré une session avec le fournisseur d'API VASA.
