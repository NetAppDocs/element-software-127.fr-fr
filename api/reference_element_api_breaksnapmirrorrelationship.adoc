...
[role="lead"]
L'interface utilisateur web d'Element utilise la `BreakSnapMirrorRelationship` méthode pour interrompre une relation SnapMirror. Lorsqu'une relation SnapMirror est rompue, le volume de destination est devenu lecture-écriture et indépendant, et peut ensuite diverger de la source. Vous pouvez rétablir la relation avec la `ResyncSnapMirrorRelationship` méthode API. Cette méthode requiert la disponibilité du cluster ONTAP.
