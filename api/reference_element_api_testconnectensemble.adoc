...
[role="lead"]
Vous pouvez utiliser cette `TestConnectEnsemble` méthode pour vérifier la connectivité avec un ensemble de bases de données spécifié. Par défaut, il utilise l'ensemble pour le cluster auquel le nœud est associé. Vous pouvez également fournir un ensemble différent pour tester la connectivité.
