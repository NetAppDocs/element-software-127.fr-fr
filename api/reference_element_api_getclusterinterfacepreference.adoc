...
[role="lead"]
Grâce à cette `GetClusterInterfacePreference` méthode, les systèmes intégrés aux clusters de stockage exécutant le logiciel Element peuvent obtenir des informations sur les préférences d'interface de cluster existantes. Cette méthode est destinée à un usage interne.
