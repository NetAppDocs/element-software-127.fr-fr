...
[role="lead"]
Vous pouvez utiliser la `EnableSnmp` méthode pour activer SNMP sur les nœuds de cluster. Lorsque vous activez SNMP, l'action s'applique à tous les nœuds du cluster et les valeurs qui sont passées remplacent toutes les valeurs définies lors de tout appel précédent à `EnableSnmp`.
