...
[role="lead"]
Vous pouvez utiliser la `ModifyClusterAdmin` méthode pour modifier les paramètres d'un administrateur de cluster, d'un administrateur de cluster LDAP ou d'un administrateur de cluster tiers fournisseur d'identité (IDP). Vous ne pouvez pas modifier l'accès au compte admin du cluster administrateur.
