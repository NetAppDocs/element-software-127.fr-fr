...
[role="lead"]
Vous pouvez utiliser la `StartBulkVolumeWrite` méthode pour démarrer une session d'écriture de volume en bloc sur un volume spécifié.
...
* `native`: Les données opaques sont renvoyées, plus petites et plus efficaces, stockées et écrites sur une écriture de volume en bloc ultérieure.
