...
[role="lead"]
Vous pouvez utiliser la `GetBootstrapConfig` méthode pour obtenir des informations sur le cluster et le nœud à partir du fichier de configuration d'amorçage. Utilisez cette méthode d'API sur un nœud individuel avant d'être associé à un cluster. Informations cette méthode renvoie est utilisée dans l'interface de configuration du cluster lors de la création d'un cluster.
