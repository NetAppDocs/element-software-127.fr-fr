...
Certains nœuds offrent des fonctionnalités de gestion hors bande supplémentaires par le biais d'un port de gestion dédié. Les nœuds NetApp H300S, H500S et H700S permettent également l'accès IPMI via le port A. dans le cadre de la meilleure pratique, vous devez faciliter la gestion à distance en configurant la gestion hors bande pour tous les nœuds de votre environnement.
