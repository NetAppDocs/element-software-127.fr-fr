...
NOTE: Vous pouvez définir le seuil d'avertissement, qui est par défaut inférieur de 3 % au seuil d'erreur. Les seuils erreur et critique sont prédéfinis et ne peuvent pas être configurés par conception. Le seuil d'erreur indique que moins d'un nœud de capacité reste dans le cluster. Pour connaître les étapes à suivre pour définir link:../storage/task_system_manage_cluster_set_the_cluster_full_threshold.html["Configuration du seuil maximum du cluster"]le seuil, reportez-vous à la section .

+

TIP: Pour plus d'informations sur l'API Element des seuils de cluster, reportez-vous à la section link:../api/reference_element_api_getclusterfullthreshold.html["« GetClusterFullThreshold »"] dans la documentation _Element Software API_. Pour plus d'informations sur la capacité des blocs et des métadonnées, reportez-vous à la section link:../storage/concept_monitor_understand_cluster_fullness_levels.html["Présentation des niveaux de remplissage du cluster"] dans la documentation du logiciel _Element_.
...
Pour plus d'informations sur la méthode API Element associée, reportez-vous à la link:../api/reference_element_api_getclusterstats.html["GetClusterStats"] méthode dans la documentation _Element Software API_.
