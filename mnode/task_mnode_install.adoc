...
*** *--storage_mvip (-sm) [Adresse MVIP]* : L'adresse IP virtuelle de gestion (MVIP) du cluster de stockage exécutant le logiciel Element. Configurez le nœud de gestion avec le même cluster de stockage que celui utilisé lors de link:task_mnode_recover.html#configure-time-sync["Configuration de serveurs NTP"].
