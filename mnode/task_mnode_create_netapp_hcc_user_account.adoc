...
* Utilisez le nouveau rôle NetApp HCC à link:task_mnode_create_netapp_hcc_role_vcenter.html#add-an-asset-to-the-management-node["ajouter une ressource"] pour le nœud de gestion.
...
Si vous devez ajouter manuellement un nouveau compte après l'installation, utilisez le nouveau compte utilisateur HCC que vous avez créé dans <<Configurez un nouveau compte utilisateur dans vCenter>>. Pour plus d'informations, voir link:task_mnode_add_assets.html["Ajoutez une ressource de contrôleur au nœud de gestion"].
